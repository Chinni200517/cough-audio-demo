"""PDF reporting and lightweight local assessment history for Auralis."""
...
def create_pdf_report(
    *, patient_id: str, email: str, age, gender: str, result_html: str,
    ...
    RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
    fd, pdf_path = tempfile.mkstemp(prefix=f"auralis-{patient_id.lower()}-", suffix=".pdf", dir=RUNTIME_DIR)
    os.close(fd)
    verification_text = f"Auralis report|{patient_id}|{label}|{confidence:.4f}|{datetime.now():%Y-%m-%d}"
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        ax.text(.06, .95, "AURALIS CARE", color="#bcecf2", fontsize=11, weight="bold")
        ...
        ax.text(.06, .06, "Digitally generated by Auralis Care", fontsize=9, weight="bold", color="#0d6075")

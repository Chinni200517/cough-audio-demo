...
def create_pdf_report(
    *, patient_id: str, email: str, age, gender: str, result_html: str,
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        # Also save high-resolution PNG for direct WhatsApp photo sharing
...
def send_prescription_email(
    ...
    """Automated delivery of rich HTML medical prescription via Gmail/SMTP."""
    import smtplib
    ...
    sender = (sender_email or os.getenv("SMTP_USER") or os.getenv("GMAIL_USER") or "").strip()
    pwd = (sender_password or os.getenv("SMTP_PASSWORD") or os.getenv("GMAIL_APP_PASSWORD") or "").strip()

    if not sender or not pwd:
        return False, (
            "Gmail SMTP credentials not set. To send directly, please provide your Gmail and 16-character Google App Password, "
            "or use the 1-click 'Open in Gmail Web' / 'Copy Rich HTML' buttons below."
        )

    try:
        msg = MIMEMultipart("mixed")
        msg["From"] = f"Team NOVIX Pulmonology <{sender}>"
        msg["To"] = to_email
        msg["Subject"] = subject
        ...
        return True, f"✅ Official Medical Prescription successfully delivered to {to_email} via Gmail/SMTP!"
